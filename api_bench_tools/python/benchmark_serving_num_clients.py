...
class BenchProcess(mp.Process):
    ...
    #     self.join()
    #     if self.outputs is None:
    #         raise RuntimeError("Benchmark process did not complete successfully")
    #     return self.outputs
...
def benchmark(
    ...
    logger.info(f"Starting benchmark for backend: {backend}, model_id: {model_id}, client_id: {client_id}, num_requests: {num_requests}, stop_time: {stop_time}")
    ...
    for request_id, request in enumerate(input_requests):
        ...
        logger.info(f"Request {request_id:4d} of client {client_id:4d}: prompt_len={prompt_len:4d}, output_len={output_len:4d}, md5={hashlib.md5(prompt.encode('utf-8')).hexdigest()[:8]}")
...
def get_input_requests(
    ...
    if args.allow_repetitive_requests:
        ...
        for client_id in range(args.num_clients):
            input_requests_i = sampled_requests[client_id:] + sampled_requests[:client_id]
            if client_id % 2 == 1:
...
def main(args: argparse.Namespace):
    ...
    for client_id in range(args.num_clients):
        ...
        logger.info(f"client {client_id} launched with ramp up time {client_id * args.ramp_up_time / args.num_clients}")
